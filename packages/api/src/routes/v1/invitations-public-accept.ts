  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const invitation = await app.withContext({ role: 'admin' as const }, async (tx) => {
        ...
        // Cheap email collision check (no race-window guarantee; the DB
        // unique constraint + P2002 catch in Phase 4 is the hard guard).
      ...
      const newUser = await app.withContext({ role: 'admin' as const }, async (tx) => {
